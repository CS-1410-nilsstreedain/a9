...
public class Sprite {
	...
	public boolean isCollidingPoint(Point2D.Double point) {
		return point.getX() >= position.getX() && point.getX() <= position.getX() + hitboxSize.getX() && // within x
																											// range
				point.getY() >= position.getY() && point.getY() <= position.getY() + hitboxSize.getY(); // within y
																										// range
	}
